...
@Component({
  selector: 'app-app-status-management',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom: 1.5rem;">
      <h3 style="margin: 0; font-size: 1.5rem; font-weight: 600;">Application Status Management</h3>
      ...
      <table class="table">
        ...
        <tbody>
          <tr *ngFor="let item of filteredItems">
            ...
            <td>
              <div style="display: flex; align-items: center; gap: 8px;">
                <div [style.background-color]="item.color" style="width: 24px; height: 24px; border-radius: 4px; border: 1px solid #e2e8f0;"></div>
                <span style="font-family: monospace; color: #64748b;">{{ item.color }}</span>
              </div>
            </td>
            <td>
          ...
          <tr *ngIf="items.length === 0">
            <td colspan="3" class="text-center text-muted py-4">No statuses found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="modal-backdrop" *ngIf="showModal">
      <div class="modal-content" style="max-width: 500px;">
        <h4 style="margin-top: 0; margin-bottom: 24px; font-weight: 600;">{{ currentItem.id ? 'Edit Status' : 'Add Status' }}</h4>
        
        <div style="margin-bottom: 16px;">
          <label style="display: block; margin-bottom: 8px; font-weight: 500; font-size: 0.9rem;">Status Name</label>
          <input type="text" class="form-control" [(ngModel)]="currentItem.name" placeholder="e.g. Approved" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; background-color: #f8fafc;">
        </div>
        
        <div style="margin-bottom: 16px;">
          <label style="display: block; margin-bottom: 8px; font-weight: 500; font-size: 0.9rem;">Color</label>
          <div style="display: flex; gap: 12px;">
            <input type="color" class="form-control form-control-color" [(ngModel)]="currentItem.color" style="height: 44px; width: 60px; padding: 4px; border-radius: 8px; border: 1px solid #cbd5e1;">
            <input type="text" class="form-control" [(ngModel)]="currentItem.color" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; background-color: #f8fafc; flex: 1;">
          </div>
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px; padding-top: 16px; border-top: 1px solid #e2e8f0;">
          ...
          <button class="btn btn-primary" (click)="saveItem()" style="padding: 10px 20px; border-radius: 8px; font-weight: 500;">Save Status</button>
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .badge { padding: 6px 12px; color: white; border-radius: 20px; font-weight: 500; font-size: 0.8rem; display: inline-block; }
...
export class AppStatusManagementComponent implements OnInit {
  items: any[] = [];
  currentItem: any = { name: '', color: '#10B981' };
  ...
  loadItems() {
    this.settingsService.getAppStatuses().subscribe({
  ...
  openModal() { this.currentItem = { name: '', color: '#10B981' }; this.showModal = true; }
  ...
  saveItem() { 
    ...
    if (this.currentItem.id) {
      this.settingsService.updateAppStatus(this.currentItem.id, this.currentItem).subscribe({
        next: (res: any) => {
          if (res.success) {
      ...
      this.settingsService.createAppStatus(this.currentItem).subscribe({
        next: (res: any) => {
          if (res.success) {
  ...
  deleteItem(id: number) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        this.settingsService.deleteAppStatus(id).subscribe({
          next: (res: any) => {
            if (res.success) {
