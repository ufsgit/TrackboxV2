...
@Component({
  ...
  template: `
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom: 1.5rem;">
      <h3 style="margin: 0; font-size: 1.5rem; font-weight: 600;">Designation Management</h3>
      ...
      <div class="card-body p-3">
        <div style="display: flex; align-items: center;">
          ...
          <div style="width: 300px; position: relative;">
            ...
            <input type="text" class="form-control" placeholder="Search designation..." [(ngModel)]="searchTerm" style="padding-left: 40px; height: 42px; width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; background-color: #f8fafc; color: #334155; outline: none;">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal-backdrop" *ngIf="showModal">
      <div class="modal-content" style="max-width: 500px;">
        <h4 style="margin-top: 0; margin-bottom: 24px; font-weight: 600;">{{ currentItem.id ? 'Edit Designation' : 'Add Designation' }}</h4>
        
        <div style="margin-bottom: 16px;">
          <label style="display: block; margin-bottom: 8px; font-weight: 500; font-size: 0.9rem;">Designation Name</label>
          <input type="text" class="form-control" [(ngModel)]="currentItem.name" placeholder="e.g. Manager" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; background-color: #f8fafc;">
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px; padding-top: 16px; border-top: 1px solid #e2e8f0;">
          ...
          <button class="btn btn-primary" (click)="saveItem()" style="padding: 10px 20px; border-radius: 8px; font-weight: 500;">Save Designation</button>
        </div>
      </div>
    </div>
  `,
...
export class DesignationManagementComponent implements OnInit {
  ...
  saveItem() { 
    ...
    if (this.currentItem.id) {
      this.settingsService.updateDesignation(this.currentItem.id, this.currentItem).subscribe({
        next: (res: any) => {
          if (res.success) {
            Swal.fire({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, icon: 'success', title: 'Designation updated' });
            this.loadItems();
            this.closeModal();
          }
        }
      ...
      this.settingsService.createDesignation(this.currentItem).subscribe({
        next: (res: any) => {
          if (res.success) {
            Swal.fire({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, icon: 'success', title: 'Designation added' });
            this.loadItems();
            this.closeModal();
          }
        }
  ...
  deleteItem(id: number) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        this.settingsService.deleteDesignation(id).subscribe({
          next: (res: any) => {
            if (res.success) {
              Swal.fire({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, icon: 'success', title: 'Designation deleted' });
              this.loadItems();
            }
          }
        });
